...
void calc_5(int connfd) {
    while (1) {
        uint8_t buffer[sizeof(int32_t) + 2 * sizeof(int64_t)];
        ssize_t size_read = read(connfd, buffer, sizeof(buffer));
        if (size_read == 0) return;
        if (size_read != sizeof(buffer)) {
            fprintf(stderr, "Incomplete PDU received\n");
            continue;
        }

        int32_t op;
        int64_t op1, op2;
        parse_pdu(buffer, &op, &op1, &op2);
